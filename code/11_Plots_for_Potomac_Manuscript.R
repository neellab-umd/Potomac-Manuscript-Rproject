...
## Population Level Genetic Diversity Graphs ####
#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX

pop.gen.stats <- read_csv(here("processed.data",
                               "final.site.level.popgen.stats.with.labels.csv")) %>%
  dplyr::mutate(Tide=ifelse(Tide =="NonTidal","Nontidal", Tide)) %>% 
  dplyr::mutate(NewPop = fct_reorder(NewPop,OrderPop),
         Tide = factor(Tide))

##This function is the template for all the violin plots.  Use with map2 to create the plots with vectors of y values and their corresponding y axis labels.   
...
##Set up themes to control inclusion of x-axis title and text on different graphs.
## Until patchwork provides a way to stack graphs and use common axes, this is the necessary workaround.
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++
## Combined Population Diversity Graph With Common X axis
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++

#  Apply violins.galore function to all our site-level population genetic data.

p <-list(violins.galore("PercPolymorphic","P")+remove_x,
         violins.galore("Ap","A")+remove_x,
         violins.galore("Ar",expression(paste(A[r])))+remove_x,
         violins.galore("Ho",expression(paste(H[O])))+remove_x,
         violins.galore("He",expression(paste(H[E])))+remove_x,
         violins.galore("Fis",expression(paste(F[IS])))+remove_x,
         violins.galore("FST.within",expression(paste(F[ST])))+remove_x_title,
         violins.galore("Gst.Hed.within",expression(paste("G\u2032\u2032"[ST]))),
         violins.galore("Josts.D.within",expression(paste(D[est])))+remove_x_title
         )
combo <- patchwork::wrap_plots(p, ncol=3) + 
  plot_annotation(tag_levels = "A",tag_suffix = ')') & 
  theme(plot.tag.position = c(.27, .91),
        plot.tag = element_text(size = 9, hjust = 0, vjust = 0))


ggsave(filename=here("figures",'Combo.Pop.Gen.png'), 
       plot = combo, 
