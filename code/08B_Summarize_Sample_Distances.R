...
ggplot.Within.Pop.Sample.Distances <-
  ...
  dplyr::mutate(FromTide = 
                ...
                FromPop = factor(FromPop),
                forcats::fct_reorder(FromPop,FromOrderPop)
                ) %>% 
  dplyr::distinct(FromSample,ToSample, .keep_all = TRUE) %>% 
  dplyr::group_by(FromPop,FromMLG,ToMLG,FromTide) %>%
  dplyr::reframe(FinalDist = identity(FinalDist)) %>% 
## Make the ggplot
  ggplot(aes(FromPop, FinalDist))+
  ...
  geom_point(data = distsumall_by.pop, aes(x = FromPop, y=MinMinDistance), 
             color = "gray80", fill = "gray80",
             shape = 25, size = 1.5) +
  geom_point(data = distsumall_by.pop, aes(x = FromPop, y=MeanMeanDistance),
             color = "gray80",fill = "gray80",
             shape = 24, size = 1.5) +
