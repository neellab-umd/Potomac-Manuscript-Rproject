...
#To run it, first source the function and then call pgen.and.sex(gid) where gid is your genind name that you have already loaded. The code where it is run for the LAT manuscript is in the script LAT_poppr_8_2021.R in the R.psex project.
...
#Calculate for each population separately - for this calculation the first encounter in each population is a probability at or near 1. So it only tells you about multiple encounters in that population.
#
  ...
  # Then calculated it regardless of population
...
#Create a final data frame that includes pgen, psex.second. Psex.multiple, the mlg ID, number of times the mlg was seen, total number of samples and mlgs,and the river it is from.
